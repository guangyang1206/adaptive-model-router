// ===========================================================================
// Lucide icons — inline SVG (Spec §7: "Lucide only", no emoji).
// Each function returns an <svg> string using currentColor so it inherits the
// surrounding text color. Sizes per Spec: 16 inline / 20 in-button / 24 standalone.
// Paths are copied verbatim from lucide@latest (ISC licensed) so we stay
// zero-dependency (no npm icon package pulled into control-plane).
// ===========================================================================

// Spec §7 icon scale: 12 (dense inline) / 14 / 16 (inline) / 18 (section head) /
// 20 (in-button) / 24 (standalone) / 32 (feature). currentColor + fixed viewBox.
export type IconSize = 12 | 14 | 16 | 18 | 20 | 24 | 32

/** Wrap raw Lucide path markup in a sized, accessible <svg>. */
function svg(size: IconSize, body: string, extraClass = ""): string {
...
export function icon(name: IconName, size: IconSize = 16, extraClass = ""): string {
  return svg(size, P[name], extraClass)
}
